"""SliceMoE Flat — non-overlapping position-fixed slice experts.

Recurrent block replaces SwiGLU with SliceMoE.
Loop order identical to Dense FFN and RTCC (CART order).
"""
import sys
...
_cfg = _load_sibling("_slice_moe_config", "config.py")
SliceMoeFlatConfig = _cfg.SliceMoeFlatConfig

_sm = _load_sibling("_slice_moe", "slice_moe.py")
SliceMoE = _sm.SliceMoE
...
class SliceBlock(nn.Module):
    """Recurrent block: MLA cross-attn (K/V from prelude e) + SliceMoE FFN."""
    def __init__(self, model_dim, n_heads, head_dim, slice_dim, expert_hidden):
        super().__init__()
        self.norm1 = RMSNorm(model_dim)
        self.attn  = MLACrossAttention(model_dim, n_heads, head_dim)
        self.norm2 = RMSNorm(model_dim)
        self.ffn   = SliceMoE(model_dim, slice_dim, expert_hidden)
...
class SliceMoeFlatModel(nn.Module):
    def __init__(self, cfg: SliceMoeFlatConfig):
        super().__init__()
        self.cfg = cfg
        d = cfg.model_dim
        ...
        self.recurrent = SliceBlock(d, cfg.n_heads_recurrent, cfg.head_dim,
                                     cfg.slice_dim, cfg.expert_hidden)
